...
export function TossImpact({ team1, team2, team1Prob, team2Prob }: TossImpactProps) {
  ...
  return (
    <motion.div
      className="bg-gradient-to-br from-[#121922]/90 to-[#0c1218]/90 backdrop-blur-xl rounded-2xl p-4 border border-slate-700/40"
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.2 }}
    >
      <div className="flex items-center gap-2 mb-3">
        <svg className="w-3.5 h-3.5 text-amber-600" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="8" cy="8" r="6" /><path d="M5 6h6M5 10h6" /></svg>
      ...
      <div className="space-y-2">
        {scenarios.map((s, i) => {
          ...
          return (
            <motion.div
              key={i}
              className="flex items-center gap-3 px-3 py-2 rounded-lg bg-white/[0.04] border border-white/10"
              initial={{ opacity: 0, x: -10 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: 0.1 + i * 0.05 }}
            >
              <span className="text-[9px] font-bold text-slate-500 w-6">{s.icon}</span>
              <span className="text-[10px] text-slate-300 flex-1 min-w-0 truncate">{s.label}</span>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <div className="w-20 sm:w-28 h-2 rounded-full overflow-hidden bg-gray-800 flex">
                  <div className="h-full rounded-l-full transition-all duration-500" style={{ width: `${s.p1 * 100}%`, backgroundColor: team1Meta.primaryColor }} />
                  <div className="h-full rounded-r-full transition-all duration-500" style={{ width: `${s.p2 * 100}%`, backgroundColor: team2Meta.primaryColor }} />
                </div>
